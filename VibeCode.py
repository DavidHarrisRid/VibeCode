...
BOARD_WIDTH = 12
BOARD_HEIGHT = 22
# Seconds between automatic piece drops
TICK_RATE = 0.5
...
SNAKE_TICK_HOR = 0.1
# Up/down moves are twice as slow as horizontal moves
...
CLIMB_VISIBLE = 20
CLIMB_JUMP = 4
# Base tick delay for movement updates
CLIMB_TICK = 0.05

# Pacman constants
PAC_TICK = 0.2  # ghost movement speed
PAC_LAYOUT = [
    "####################",
    "#P.......##........#",
    "#.####.#.##.#.####.#",
...
class Pacman:
    """Very small Pacman clone with one ghost."""

    def __init__(self):
        self.board = [list(row) for row in PAC_LAYOUT]
        self.ghosts = []
        self.score = 0
        self.pellets = 0
    ...
    def move_ghosts(self):
        for g in self.ghosts:
            ...
            if options:
                g[0], g[1] = random.choice(options)

    def check_collisions(self):
        for x, y in self.ghosts:
            if x == self.pac_x and y == self.pac_y:
                self.game_over = True
    ...
    def draw(self, stdscr):
        ...
        for x, y in self.ghosts:
            stdscr.addstr(y, x, 'M')
        stdscr.addstr(self.pac_y, self.pac_x, 'C')
        stdscr.addstr(PAC_HEIGHT, 0, f'Score: {self.score}')
        stdscr.refresh()
    ...
    def run(self, stdscr):
        ...
        while not self.game_over and not self.win:
            ...
            now = time.time()
            if now - self.last_move > PAC_TICK:
                self.move_ghosts()
                self.last_move = now
        ...
        stdscr.addstr(PAC_HEIGHT + 1, 0, msg + ' - press any key')
        stdscr.refresh()
        stdscr.getch()
